...
# Randomized response only applied to lower triangular matrix, and copy the lower triangular matrix to upper triangular matrix
# This way we ensure that the matrix is symmetric
class Client():
    ...
    def AddLDP(self) -> Tuple[torch.Tensor]:
        ...
        def rr_adj() -> torch.Tensor:
            ...
            res = res.tril(diagonal=1) + res.tril(diagonal=1).t()
