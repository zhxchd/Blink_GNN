...
def run_rr(graph, linkless_graph, model_name, eps, hp, num_trials, use_dense_model=False):
    val_loss = np.zeros(num_trials)
    test_acc = np.zeros(num_trials)
    for i in range(num_trials):
        client = rr.Client(eps=eps, data=graph)
        server = rr.Server(eps=eps, data=linkless_graph, use_dense_model=use_dense_model)
