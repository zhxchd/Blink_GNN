...
for dataset in ["lastfm"]:
    for model in ["gcn"]:
        ...
        plt.plot(x, [blink_res["hard"][dataset][model]["None"][0]*100 for i in x], linestyle=':', marker=" ", color="#b2df8a", label="$\epsilon=\infty$")

        # draw pure privacy MLP (lower bound)
        plt.plot(x, [blink_res["hard"][dataset]["mlp"]["None"][0]*100 for i in x], linestyle=':', marker=" ", color="#fb9a99", label="$\epsilon=0$")

        plot_utils.plot(x, [blink_res["hard"][dataset][model][i][0]*100 for i in x], [blink_res["hard"][dataset][model][i][1]*100 for i in x], color="b", label="Blink-Hard", fill=False)
        if model != "gat":
            plot_utils.plot(x, [blink_res["soft"][dataset][model][i][0]*100 for i in x], [blink_res["soft"][dataset][model][i][1]*100 for i in x], color="r", label="Blink-Soft", fill=False)
        plot_utils.plot(x, [blink_res["hybrid"][dataset][model][i][0]*100 for i in x], [blink_res["hybrid"][dataset][model][i][1]*100 for i in x], color="g", label="Blink-Hybrid", fill=False)
        ...
        plt.savefig(f"figures/{dataset}_{model}_standalone.pdf", bbox_inches='tight')
        legend=plt.legend()
        ax = plt.gca()
...
fig.savefig("figures/lastfm_gcn_legend.pdf", bbox_inches=bbox)